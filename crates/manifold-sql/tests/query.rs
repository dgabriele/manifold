...
fn select_with_arithmetic_expression() {
    let (db, _dir) = test_db();
    db.execute(
        "CREATE TABLE t (id INTEGER, price INTEGER, qty INTEGER)",
        &[],
    )
    .unwrap();
    ...
    let result = db.query("SELECT price * qty AS total FROM t", &[]).unwrap();
...
fn setup_where_table(db: &manifold_sql::Database) {
    ...
    .unwrap();
    db.execute(
        "INSERT INTO items (id, name, price, category) VALUES (1, 'apple', 5, 'fruit')",
        &[],
    )
    .unwrap();
    db.execute(
        "INSERT INTO items (id, name, price, category) VALUES (2, 'banana', 3, 'fruit')",
        &[],
    )
    .unwrap();
    db.execute(
        "INSERT INTO items (id, name, price, category) VALUES (3, 'carrot', 4, 'vegetable')",
        &[],
    )
    .unwrap();
    db.execute(
        "INSERT INTO items (id, name, price, category) VALUES (4, 'donut', 8, 'pastry')",
        &[],
    )
    .unwrap();
    db.execute(
        "INSERT INTO items (id, name, price, category) VALUES (5, 'eclair', 10, 'pastry')",
        &[],
    )
    .unwrap();
}
...
fn where_comparison_lt_gt_le_ge() {
    ...
    let lt = db
        .query("SELECT COUNT(*) FROM items WHERE price < 5", &[])
        .unwrap();
...
fn where_between() {
    ...
    let result = db
        .query(
            "SELECT COUNT(*) FROM items WHERE price BETWEEN 4 AND 8",
            &[],
        )
        .unwrap();
...
fn where_like_underscore() {
    ...
    db.execute("INSERT INTO t (val) VALUES ('cat')", &[])
        .unwrap();
    db.execute("INSERT INTO t (val) VALUES ('car')", &[])
        .unwrap();
    db.execute("INSERT INTO t (val) VALUES ('card')", &[])
        .unwrap();
...
fn left_join_with_null() {
    ...
    let carol_row = result
        .rows()
        .iter()
        .find(|r| r.get::<String>(0).unwrap() == "carol")
        .unwrap();
...
fn right_join_with_null() {
    ...
    db.execute("INSERT INTO b (id, a_id, val) VALUES (1, 1, 'match')", &[])
        .unwrap();
    db.execute(
        "INSERT INTO b (id, a_id, val) VALUES (2, 99, 'orphan')",
        &[],
    )
    .unwrap();
...
fn cross_join() {
    let (db, _dir) = test_db();
    db.execute("CREATE TABLE x (id INTEGER, v TEXT)", &[])
        .unwrap();
    db.execute("CREATE TABLE y (id INTEGER, w TEXT)", &[])
        .unwrap();
...
fn multi_table_join_three_tables() {
    ...
    .unwrap();
    db.execute(
        "CREATE TABLE salaries (id INTEGER PRIMARY KEY, emp_id INTEGER, amount INTEGER)",
        &[],
    )
    .unwrap();

    db.execute("INSERT INTO departments (id, name) VALUES (1, 'eng')", &[])
        .unwrap();
    db.execute(
    ...
    .unwrap();
    db.execute(
        "INSERT INTO salaries (id, emp_id, amount) VALUES (1, 1, 100000)",
        &[],
    )
    .unwrap();
...
fn setup_agg_table(db: &manifold_sql::Database) {
    db.execute(
        "CREATE TABLE sales (id INTEGER, category TEXT, amount INTEGER)",
        &[],
    )
    .unwrap();
    db.execute(
        "INSERT INTO sales (id, category, amount) VALUES (1, 'A', 10)",
        &[],
    )
    .unwrap();
    db.execute(
        "INSERT INTO sales (id, category, amount) VALUES (2, 'A', 20)",
        &[],
    )
    .unwrap();
    db.execute(
        "INSERT INTO sales (id, category, amount) VALUES (3, 'B', 30)",
        &[],
    )
    .unwrap();
    db.execute(
        "INSERT INTO sales (id, category, amount) VALUES (4, 'B', 40)",
        &[],
    )
    .unwrap();
    db.execute(
        "INSERT INTO sales (id, category, amount) VALUES (5, 'B', 50)",
        &[],
    )
    .unwrap();
}
...
fn order_by_multiple_columns() {
    ...
    db.execute("INSERT INTO t (a, b) VALUES (1, 1)", &[])
        .unwrap();
    db.execute("INSERT INTO t (a, b) VALUES (2, 1)", &[])
        .unwrap();
...
fn order_by_nulls_first() {
    ...
    db.execute("INSERT INTO t (id, val) VALUES (2, 10)", &[])
        .unwrap();
...
fn index_scan_point_lookup() {
    ...
    let result = db.query("SELECT name FROM t WHERE id = 500", &[]).unwrap();
    assert_eq!(result.row_count(), 1);
...
fn index_scan_point_lookup_not_found() {
    ...
    let result = db.query("SELECT name FROM t WHERE id = 999", &[]).unwrap();
    assert_eq!(result.row_count(), 0);
}
...
fn index_scan_explain_shows_index() {
    ...
    db.execute("INSERT INTO t (id, name) VALUES (1, 'a')", &[])
        .unwrap();
    // PK equality produces RowidLookup (direct key access, faster than IndexScan)
    let plan_text = db.explain("SELECT name FROM t WHERE id = 1").unwrap();
    assert!(
        plan_text.contains("RowidLookup"),
        "EXPLAIN should show RowidLookup, got: {plan_text}"
    );
}
