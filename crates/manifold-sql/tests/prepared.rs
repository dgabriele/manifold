...
fn prepared_multiple_executions() {
    ...
    for i in 1..=100 {
        insert_stmt
            .execute(&db, &[Value::Integer(i), Value::Integer(i * 10)])
            .unwrap();
    }

    let query_stmt = db.prepare("SELECT value FROM t WHERE id = $1").unwrap();
...
fn prepared_update() {
    ...
    let stmt = db.prepare("UPDATE t SET name = $1 WHERE id = $2").unwrap();
    assert!(!stmt.is_query());
