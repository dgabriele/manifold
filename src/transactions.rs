...
impl WriteTransaction {
    ...
    fn commit_inner(&mut self) -> Result<(), CommitError> {
        ...
        // Append to WAL if enabled (BEFORE the actual commit)
        // WAL records user_root and freed/allocated pages for crash recovery.
        // system_root is computed internally by the commit methods and recorded as None here;
        // during WAL replay, apply_wal_transaction uses the user_root to restore state.
        ...
        match self.durability {
            ...
            InternalDurability::Immediate => {
                if self.wal_journal.is_some() {
                    // WAL already fsynced, use non-durable commit to just make changes visible
                    self.non_durable_commit(user_root, allocated_pages, stored_data_freed_pages)?;
                    self.apply_savepoint_state_on_commit();
                } else {
