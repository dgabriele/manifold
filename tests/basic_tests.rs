...
fn tuple_type_function_lifetime() {
    #[allow(dead_code)]
    fn insert_inferred_lifetime(table: &mut manifold::Table<(&str, u8), u64>) {
        table
    ...
    fn insert_explicit_lifetime<'a>(table: &mut manifold::Table<(&'a str, u8), u64>) {
        table
            .insert(&(String::from("hello").as_str(), 8), &1)
            .unwrap();
    }
...
fn bulk_insert_unsorted() {
    ...
    let txn = db.begin_write().unwrap();
    {
        ...
        assert_eq!(table.get(&2).unwrap().unwrap().value(), 4);
    }
    txn.commit().unwrap();
}

#[test]
fn bulk_insert_large_unsorted() {
    let tmpfile = create_tempfile();
    let db = Database::create(tmpfile.path()).unwrap();
    let txn = db.begin_write().unwrap();
    {
        ...
        let data: Vec<(u64, u64)> = (0..25000)
            .map(|_| {
                let key = rng.random_range(0..100000);
                (key, key * 2)
            })
            .collect();
...
fn bulk_remove() {
    ...
    let txn = db.begin_write().unwrap();
    {
        ...
        assert!(table.get(&75).unwrap().is_some());
    }
    txn.commit().unwrap();
}

#[test]
fn bulk_remove_nonexistent() {
    let tmpfile = create_tempfile();
    let db = Database::create(tmpfile.path()).unwrap();
    let txn = db.begin_write().unwrap();
    {
