...
mod stress {
    ...
    // ========================================================================
    // DEFERRED FLUSH STRESS TESTS
    // ========================================================================

    /// Verifies deferred flush: writes visible via get() within same txn and after commit.
    #[test]
    fn stress_deferred_flush_read_your_writes() {
        ...
        // Read via new read transaction (sees memtable snapshot)
        let rtxn = cf.begin_read().unwrap();
        let t = rtxn.open_table(TABLE_U64).unwrap();
        for &k in &keys {
            let val = t.get(&k).unwrap();
    ...
    /// Uses `&[u8]` table types because WAL recovery replays with untyped bytes.
    #[test]
    fn stress_deferred_flush_persistence() {
        const TABLE_BYTES_KV: TableDefinition<&[u8], &[u8]> =
            TableDefinition::new("deferred_bytes");
        ...
        // Pre-create the table without deferred flush so the schema exists in the B-tree.
        {
            ...
            let cf = db.column_family("cf").unwrap();
            let txn = cf.begin_write().unwrap();
            let t = txn.open_table(TABLE_BYTES_KV).unwrap();
            drop(t);
            txn.commit().unwrap();
        }

        // Write data with deferred flush
        {
            let db = ColumnFamilyDatabase::builder()
                .deferred_flush(true)
                .open(&db_path)
        ...
        // Reopen and verify (WAL recovery should replay LogicalOps)
        {
            let db = ColumnFamilyDatabase::builder()
                .deferred_flush(true)
                .open(&db_path)
    ...
    /// Verifies concurrent CF writers with deferred flush all succeed.
    /// Each thread holds its CF handle for the duration so the memtable is shared
    /// between the writer and the verification read.
    #[test]
    fn stress_deferred_flush_concurrent_cf_writers() {
        ...
        let handles: Vec<_> = (0..num_cfs)
            .map(|cf_id| {
                ...
                thread::spawn(move || {
                    barrier.wait();
                    // Hold a single CF handle for both writing and verification
                    let cf = db.column_family(&format!("cf_{}", cf_id)).unwrap();
                    for i in 0..writes_per_cf {
                        let txn = cf.begin_write().unwrap();
                        let mut t = txn.open_table(TABLE_U64).unwrap();
                    ...
                    // Verify within the same thread using the same CF handle
                    let rtxn = cf.begin_read().unwrap();
                    let t = rtxn.open_table(TABLE_U64).unwrap();
                    for i in 0..writes_per_cf {
                        let val = t.get(&i).unwrap();
