use manifold::{ReadableDatabase, ReadableTableMetadata};

const ELEMENTS: usize = 3;

trait TestData: manifold::Value + redb2_6::Value {
    fn make_data_v2_6<'a>() -> [<Self as redb2_6::Value>::SelfType<'a>; ELEMENTS]
    where
        Self: 'a;

    fn make_data<'a>() -> [<Self as manifold::Value>::SelfType<'a>; ELEMENTS]
    where
        Self: 'a;
}
...
fn test_helper<K: TestData + manifold::Key + redb2_6::Key + 'static, V: TestData + 'static>() {
    {
        ...
        let db = manifold::Database::open(tmpfile.path()).unwrap();
        let read_txn = db.begin_read().unwrap();
        let table_def: manifold::TableDefinition<K, V> = manifold::TableDefinition::new("table");
        let table = read_txn.open_table(table_def).unwrap();
        assert_eq!(table.len().unwrap(), ELEMENTS as u64);
        for i in 0..ELEMENTS {
            let result = table.get(&K::make_data()[i]).unwrap().unwrap();
            let value = result.value();
            let bytes = <V as manifold::Value>::as_bytes(&value);
            let expected = &V::make_data()[i];
            let expected_bytes = <V as manifold::Value>::as_bytes(expected);
